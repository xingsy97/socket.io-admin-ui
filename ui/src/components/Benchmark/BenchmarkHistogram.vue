  ...
  export default defineComponent({
    ...
    data() {
        return {
            ...
            maxClients: 1,
            emitPerSecond: 50,
            clientCreationIntervalInMs: 100,
    ...
    setup() {
      ...
      const option = reactive({
        ...
        title: { text: 'Benchmark Chart, 1 Round Trip = Client -> Server -> Client ', left: '10vh', top: "20vh"},
        ...
        legend: {
            ...
            data: ['Clients Received Packet Per Second', 'Clients Sent Packet Per Second', 'Max Round Trip Time Per Second']
        },
        series: [
          {
            name: "Clients Received Packet Per Second",
          ...
          {
            name: "Clients Sent Packet Per Second",
          ...
          {
            name: "Max Round Trip Time Per Second",
    ...
    methods: {
        startBenchmark(serviceEndpoint, namespace, wsOnly, path, parser) {
            ...
            const createClient = () => {
                ...
                const socket = useWps ? io(serviceEndpoint + namespace, { transports: transports, path: path })
                ...
                socket.on("connect", () => {
                    ...
                    this.timeTasks.push(setInterval(() => {
                            socket.emit("client to server event", (++idx).toString() + "," + new Date().getTime().toString());
                            lifetimeData.totalEmittedPackets++;
                        //}, 1000 / EMIT_PER_SECOND * _));
                        }, 1000 / EMIT_PER_SECOND * 1));
                });

                socket.on("server to client event", (data) => {
                    var idx = data.split(",")[0]; var timestamp = data.split(",")[1];
